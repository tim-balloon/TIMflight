...
{
	return le ? le32toh(*(uint32_t*)p) : be32toh(*(uint32_t*)p);
}
...
TAILQ_HEAD(bit_copy_queue, bit_copy_queue_entry);
...
struct bit_copy_queue_entry {
	...
	unsigned src_offset;
	unsigned bit_count;
    TAILQ_ENTRY(bit_copy_queue_entry) list;
};
