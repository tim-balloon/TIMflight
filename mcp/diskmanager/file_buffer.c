...
static const unsigned int filebuffer_expansion_unit = 32768;/* What overage should we include in a re-allocation (bytes) */
static const float filebuffer_historical_weight = 0.01;		/* How much do we discount the recent size reqs in the average */
static const long int nano_wait = 250000000;				/* 0.25s in nanoseconds.  Timeout for consolidation lock */
...
int filebuffer_init(filebuffer_t *m_buffer, size_t m_len)
{
	...
	m_buffer->buf = balloc(loglevel_nolog, m_buffer->alloc);
	...
	{
		berror(loglevel_nolog, "file_buffer::filebuffer_init : Could not allocate buffer of size %u", (unsigned)m_buffer->alloc);
		return -1;
	}
	e_memset(m_buffer->buf, 0, m_buffer->alloc);
	if(pthread_mutex_init(&(m_buffer->append_lock),NULL) == -1)
	{
		berror(loglevel_nolog, "file_buffer::filebuffer_init : Could not initialize append mutex");
		return -1;
	}
	if(pthread_mutex_init(&(m_buffer->consume_lock), NULL) == -1)
	{
		berror(loglevel_nolog, "file_buffer::filebuffer_init : Could not initialize consume mutex");
...
ssize_t filebuffer_append(filebuffer_t *m_buffer, const char *m_data, size_t len)
{
	...
	{
		ebex_nolog("Failed to append data to buffer");
	}
...
static ssize_t filebuffer_append_space(filebuffer_t *m_buffer, char **m_datap, size_t len)
{
	...
	while (m_buffer->end + len >= m_buffer->alloc)
	{
		...
		if (filebuffer_realloc(m_buffer, len) == -1)
		{
			...
			{
				berror(loglevel_nolog, "file_buffer::filebuffer_append_space : Could not expand buffer");
				return -1;
			}
		}
	}
	...
	*m_datap = m_buffer->buf + m_buffer->end;
	...
	{
		*m_datap = NULL;
		ebex_nolog("Returning NULL buffer.");
		return -1;
	}
	return (ssize_t)len;
...
static int filebuffer_realloc(filebuffer_t *m_buffer, size_t len)
{
	...
	ebex_nolog("Failed to allocate %u bytes", (unsigned) m_buffer->alloc);
...
static void filebuffer_consume(filebuffer_t *m_buffer, size_t len)
{
	...
	if (buffer_len < len)
	{
		ebex_nolog("Tried to eat more data than are in the buffer");
