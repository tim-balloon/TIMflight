...
static LIBUSB_CALL void biphase_write_cb(struct libusb_transfer * biphase_write_transfer)
{
    ...
    if (!mpsse_closing) {
        ...
        if (reader_done) {
            ...
            // Ian 5/28/2026
            // adding a conditional to ignore the WD card in charge if we decide to bypass it via command
            if (CommandData.bypass_HW_WD == 0) {
...
void biphase_writer(void * arg)
{
    ...
    while (!setup_mpsse(&ctx, serial, direction) && !shutdown_mcp) {
        ...
        if (!warned_mpsse) {
            ...
            if (InCharge) {
                blast_info("Defaulting to in charge");
            } else {
                blast_info("Defaulting to not in charge");
            }
        }
        warned_mpsse = 1;
        sleep(5);
    ...
    while (!shutdown_mcp) {
        ...
        if (ll != ll_old) {
            if (ll) {
                blast_info("BI0 linklist set to \"%s\"", ll->name);
            } else {
                blast_info("BI0 linklist set to NULL");
            }
        }
        ll_old = ll;
